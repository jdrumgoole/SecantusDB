...
//! comparison/equality-heavy `$sort`/`$group`/`$unwind`/… still being worked
//! on) or any inner expression defers, the entire `apply_pipeline` returns
//! `Fallback` and the pure-Python pipeline runs instead.
//!
...
use crate::{expressions, paths, query};
...
fn apply_stage(
    ...
    match name {
        ...
        // $sort / $group / $unwind / storage-backed / $sample / … -> Python.
        _ => Err(Fallback),
    }
}
