...
fn coll_arg(doc: &Document, cmd: &str) -> Result<String, CommandError> {
    match doc.get(cmd) {
...
fn doc_field(doc: &Document, key: &str) -> Document {
    doc.get(key)
...
fn bool_field(doc: &Document, key: &str, default: bool) -> bool {
    doc.get(key).and_then(Bson::as_bool).unwrap_or(default)
}

/// Coerce any BSON number to `i64` (for `skip` / `limit` / `index`).
...
fn write_error(index: usize, err: StorageError) -> Document {
    match err {
...
/// commands like `count`).
fn command_error(err: StorageError) -> CommandError {
    match err {
