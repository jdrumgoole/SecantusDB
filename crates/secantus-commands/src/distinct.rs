...
//! collect the distinct values. **Collation is deferred** (dedup is by BSON
//! equality; the Rust engines don't honour collation yet), so a case-insensitive
//! `distinct` would over-report — tracked with the other collation gaps.
...
pub fn distinct(doc: &Document, ctx: &mut CommandContext) -> HandlerResult {
    ...
    let bytes = storage
        .find(&ctx.db_name, &coll, &filter, None, None)
    ...
    for b in bytes {
        ...
        match get_path(&d, &key) {
            ...
            Some(Bson::Array(arr)) => {
                for e in arr {
                    push_distinct(&mut values, e);
                }
            }
            Some(v) => push_distinct(&mut values, v),
            None => {}
        }
    }
...
fn push_distinct(values: &mut Vec<Bson>, v: &Bson) {
