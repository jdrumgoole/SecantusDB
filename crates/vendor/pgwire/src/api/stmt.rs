...
pub struct StoredStatement<S> {
    ...
    /// The RAW parameter type oids exactly as they arrived in the `Parse`
    /// message, before `Type::from_oid` mapping. `parameter_types` loses a
    /// non-builtin oid (a user composite / enum / range) to `None`, because
    /// `Type::from_oid` only knows builtins; this field preserves the oid so a
    /// backend can resolve the user type against its own catalog. Same length
    /// and order as `parameter_types`. (SecantusDB local patch.)
...
impl<S> StoredStatement<S> {
        ...
        Q: QueryParser<Statement = S>,
    {
        ...
        Ok(StoredStatement {
            ...
            // Preserve the raw oids alongside the mapped types. (SecantusDB
            // local patch.)
            parameter_oids: parse.type_oids.clone(),
        })
    }
}
