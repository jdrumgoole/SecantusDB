...
**In scope:** the subset of the MongoDB wire protocol that `pymongo` actually emits — connection handshake, CRUD ops, cursors, basic commands.
...
- `src/fongodb/server.py` — `FongoDBServer`: TCP accept loop on a daemon thread, one daemon thread per connection. Owns the `Storage`. Per-request, builds a fresh `CommandContext(storage, db_name)` and calls `dispatch`.
...
- `src/fongodb/commands.py` — single dispatch table keyed on the first key of the request doc. Handshake (`hello`/`isMaster`/`ping`/`buildInfo`/etc.) and CRUD (`insert`/`find`/`update`/`delete`/`count`/`drop`/`aggregate`/`listCollections`/...). Unknown commands return `{ok: 0, code: 59, codeName: "CommandNotFound"}` so the connection survives.
- `src/fongodb/query.py` — pure `matches(doc, filter)`. Operators: `$eq`/`$ne`/`$gt`/`$gte`/`$lt`/`$lte`/`$in`/`$nin`/`$exists`/`$and`/`$or`/`$nor`/`$not`. Dotted paths walk into both maps and arrays (numeric index, or fan-out across element subdocs).
...
- `src/fongodb/storage.py` — SQLite-backed store. Schema:
  ...
  - `_fongodb_documents(db_name, coll_name, id_key BLOB, doc BLOB)` — full document is `bson.encode(doc)`; `id_key` is `bson.encode({"_": _id})` so byte-equality matches BSON-equality within a type. `RLock`-serialized; `check_same_thread=False` connection.
...
**Known limitation:** the numeric type bridge for `_id` uniqueness (where `Int32(1) == Double(1.0)` for `_id` purposes) is not implemented. Two documents with `_id: 1` (int32) and `_id: 1.0` (double) will both insert here; real MongoDB rejects the second.
...
- The on-disk repo path is still `/Users/jdrumgoole/GIT/fongo` (legacy from before the rename); the package and PyPI name are `fongodb`. Renaming the directory is fine if Joe wants to do it, but isn't required.

## Conventions for changes here

- New CRUD operators or aggregation stages should land with both a unit test in `tests/test_query.py` / `tests/test_update.py` and a `pymongo`-driven integration test in `tests/test_crud.py`. The integration test is the conformance proof; the unit test pins the semantics.
- The wire-protocol layer never ought to know about commands. The command-dispatch layer never ought to know about SQL. Keep these separations clean — they're what let us swap storage backends or add cursor pagination without rewriting everything.
