...
- **Driver-conformance gauges** — eight gauges run **unmodified** upstream tests against SecantusDB: pymongo (embedded; the headline "MongoDB compatibility" number), and mongo-go-driver / mongo-node-driver / mongo-java-driver / mongo-ruby-driver / mongo-rust-driver / mongo-php-library (PHPUnit) / mongo-php-driver (`.phpt` C extension) (each a daemon subprocess via `MONGODB_URI`). The other-language gauges catch wire-protocol bugs that pymongo's permissive client misses (e.g. cursor.id-must-be-int64); the PHP-extension and Go gauges are the strictest wire-protocol checks. Invocation (`invoke validate{,-go,-node,-java,-ruby,-rust,-php-lib,-php-ext,-all}`), include paths, language toolchain requirements, and current per-gauge caveats live in `/conformance-gauges`. All gauge dirs are dev-only (excluded from sdist/wheel). Validation runs weekly on `.github/workflows/validate.yml`.
