...
Still missing: mixed-direction compound indexes, range/`$in` operator forms on the leading field of a compound index without a single-field index, and `partialFilterExpression` / `collation`.
