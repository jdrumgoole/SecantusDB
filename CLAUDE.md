...
- **Never use `FakeStorage` (`tests/sqlfake.py`) in new tests — always the real `Storage`.** A mock storage lets a test pass while diverging from what WiredTiger actually does (row/type round-trips, transactions, cross-session visibility, persistence). Build a real `Storage(str(tmp_path))` (close it in the fixture teardown), seed it via `run_sql` with an unrestricted `Session`, and drive the SQL/PG-server code against that. This is a database — tests prove behaviour against the real engine or they prove nothing. The pre-existing `FakeStorage` usages are legacy to be migrated, not a pattern to copy.
