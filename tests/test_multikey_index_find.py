...
returns no results, because SecantusDB does not generate multikey index
...
returns the enclosing document. On SecantusDB the same index yields
`nReturned: 0` because no keys are ever generated for values inside the
array. Forcing a collection scan (`hint=[("$natural", 1)]`) returns the
document, proving the docs are stored correctly and only the index path
is broken.

Fails against SecantusDB 0.6.0b0. Should pass once multikey indexing
covers dotted-path indexes on arrays of subdocuments.
"""
...
@pytest.fixture
def collection():
    """Fresh collection on an ephemeral in-memory SecantusDB instance."""
...
def test_dotted_path_into_array_matches_via_index_scan(collection):
    ...
    via_scan = list(
        collection.find({"prices.owner_id": owner}).hint([("$natural", 1)])
    )
    via_index = list(collection.find({"prices.owner_id": owner}))

    assert len(via_scan) == 1, (
        "sanity: the document should be findable at all — collection scan "
        "must return it"
    )
...
def test_dotted_path_matches_when_target_is_not_first_element(collection):
    ...
    assert len(result) == 1, (
        "index scan should locate the document via any element of the "
        "array, not only the first"
    )
...
def test_dotted_path_matches_after_indexing_a_pre_existing_document(collection):
    ...
    result = list(collection.find({"prices.owner_id": owner}))
    assert len(result) == 1, (
        "index built after the document must still index the values "
        "inside its array"
    )
