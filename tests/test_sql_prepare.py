...
def test_prepare_no_params_and_execute():
    st, sess = _fresh()
...
def test_prepare_with_typed_params():
    st, sess = _fresh()
...
def test_prepare_multiple_params_mixed_types():
    st, sess = _fresh()
    _run(
        st,
        sess,
...
def test_prepare_untyped_params():
...
def test_execute_prepared_insert():
    st, sess = _fresh()
...
def test_execute_reuses_prepared_plan_repeatedly():
    st, sess = _fresh()
...
def test_duplicate_prepare_name_rejected():
    st, sess = _fresh()
...
def test_execute_unknown_statement_rejected():
    st, sess = _fresh()
    with pytest.raises(errors.SQLError) as exc:
...
def test_execute_wrong_argument_count_rejected():
    st, sess = _fresh()
...
def test_deallocate_unknown_is_noop():
...
def test_deallocate_quoted_name():
    st, sess = _fresh()
