...
def test_javascript_sorts_between_regex_and_maxkey(coll):
    # mongod's cross-type sort order. `Code` subclassing `str` had it sorting
    # among the strings.
    order = [d["_id"] for d in coll.find({"_id": {"$in": [1, 2, 5, 6, 8]}}).sort("v", 1)]
    assert order == [2, 6, 5, 1, 8]


def test_collated_sort_over_a_javascript_value_does_not_crash(coll):
    # `sort_levels` is lru_cached and `Code` is unhashable, so this answered
    # `1 internal server error`.
    order = [
        d["_id"]
        for d in coll.find({"_id": {"$in": [1, 2, 5]}})
        .sort("v", 1)
