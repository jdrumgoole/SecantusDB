...
def render(xml_dir: Path, out_path: Path) -> None:
    ...
    md.append(
        ...
        "tag with zero local edits. `java_validation/runner.py` finds a "
        "free port, spawns `python -m secantus --host 127.0.0.1 --port "
        "<free> --storage-path :memory:` as a subprocess, then invokes "
        "the driver's bundled gradle wrapper "
        "(`./gradlew --no-daemon -Dorg.mongodb.test.uri=mongodb://...`) "
        "for the in-scope modules in `java_validation/include_modules.py`. "
        "The system property is the seam Java's `ClusterFixture` test "
        "infrastructure reads (`org.mongodb.test.uri`); Gradle forwards "
        "it to the test JVM."
    )
    md.append("")
    md.append(
        "The driver writes JUnit XML to "
        "`<module>/build/test-results/test/TEST-*.xml`; we copy those out "
        "of the vendored tree (so the submodule stays untouched) and parse "
        "them here. Tests gated on a real `mongod` topology that "
        "SecantusDB doesn't aim to provide are out of scope and excluded "
        "from `include_modules.py` rather than letting them fail."
    )
    md.append("")
