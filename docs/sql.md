...
two-table `INNER` or `LEFT` composite** (`A [INNER|LEFT] JOIN B ON … RIGHT|FULL
JOIN C ON …`, the outer `ON` referencing `C` and exactly one of `A`/`B`) — the
`A⋈B` composite is computed atomically by a nested `$lookup` (a flat reversal
would leak half-matches, so this is the only sound lowering). `RIGHT` drives
from `C` and preserves it; `FULL` unions the forward `(A⋈B) LEFT JOIN C` main
branch with a `$unionWith` anti-branch of the `C` rows whose composite is empty.
For an `A LEFT JOIN B` composite the far side is preserved when the outer `ON`
targets the base `A`, so a `B`-less `A` row still contributes. A chain with a
`RIGHT` `ON` that reaches a non-adjacent table, a second `FULL` in the tail, a
trailing outer join over a 3-table composite, and an outer `ON` spanning *both*
composite tables stay `0A000`:
