...
argument through the join resolver, and **under `GROUP BY ROLLUP` / `CUBE` /
`GROUPING SETS`** (single-table and over a JOIN): each grouping set's branch
carries the accumulator and the statistical / bitwise finish runs over the
unioned rows.
...
A *computed* grouping key over a JOIN (`GROUP BY ROLLUP(lower(d.label))`) is
lowered through the join resolver into a synthetic field materialised on the
join prefix, so it works alongside `HAVING`, `GROUPING()`, and a window too. A
...
supported between **two** tables. Three multi-table shapes also work: a
**pure-`RIGHT` chain of 3+ tables** (every join `RIGHT`, each `ON` joining a
table to the immediately-prior one) is reversed into a `LEFT` chain driven from
...
join looks its table up over it; and a **trailing `RIGHT` or `FULL` join over an
`N`-table `INNER`/`LEFT` composite** (`A [INNER|LEFT] JOIN B ON … [[INNER|LEFT]
JOIN … ON …] RIGHT|FULL JOIN C ON …` — any number of leading `INNER`/`LEFT`
joins) — lowered by the **main ∪ anti** decomposition: the *main* branch builds
the composite by the ordinary forward pipeline (natural root `A`) and joins `C`
last (`INNER` for `RIGHT`, `LEFT` for `FULL`); the *anti* branch `$unionWith`s
the `C` rows whose forward composite is empty, null-padded. Because the composite
is always built forward from `A` (never re-rooted), this is sound for `INNER` and
`LEFT` composites of any size, so a `B`-less `A` row still contributes under a
`LEFT` composite, and the outer `ON` may reference `C` together with *any* subset
of the composite tables. A chain with a `RIGHT` `ON` that reaches a non-adjacent
table, a second `FULL` in the tail, and a composite whose own joins aren't
adjacent (each `ON` must join its table to an already-joined one) stay `0A000`:
...
`n FOLLOWING` offset **or** an `INTERVAL` offset over a `date`/`timestamp` key
(e.g. `RANGE BETWEEN INTERVAL '1 day' PRECEDING AND CURRENT ROW`). A row is
in-frame when its `ORDER BY` key is within the offset of the current row's key —
a value window, not a row count; Postgres requires exactly one `ORDER BY` column
for an offset `RANGE` frame (an interval offset additionally needs a temporal
key):
