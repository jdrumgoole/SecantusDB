...
PELICAN_PROD_CMD = (
    f"{PYTHON} -m pelican {HERE / 'content'} -o {OUTPUT} -s {HERE / 'publishconf.py'}"
)
PELICAN_SERVE_CMD = (
    f"{PYTHON} -m pelican --listen --autoreload --port 8000 -s pelicanconf.py -o output content"
)
...
# Admin-console screenshots shown on the landing page. Named here rather
# than globbed: docs/screenshots/ holds all 22 admin pages and the site
# shows four, so a glob would drag 5 MB into every build. Adding one means
# adding it here *and* as an <img> in the theme's page.html — the pair is
# checked by tests/test_docs_screenshots.py.
SCREENSHOT_SRC = REPO_ROOT / "docs" / "screenshots"
SITE_SCREENSHOTS = (
    "admin-dashboard.png",
    "admin-query.png",
    "admin-changestream.png",
    "admin-explain.png",
)


def _copy_screenshots() -> None:
    """Mirror the landing page's admin screenshots into the theme.

    Same arrangement as the brandkit assets: ``static/img/`` is a build-time
    mirror and isn't tracked, so these live once in ``docs/screenshots/``
    (where Sphinx and the README read them too) and are copied in at build
    time. One source, regenerated by ``invoke admin-screenshots``, so the
    site can't drift from the docs.
    """
    dest = THEME_IMG / "screenshots"
    dest.mkdir(parents=True, exist_ok=True)
    missing = [n for n in SITE_SCREENSHOTS if not (SCREENSHOT_SRC / n).is_file()]
    if missing:
        raise SystemExit(
            f"missing admin screenshot(s) in {SCREENSHOT_SRC.relative_to(REPO_ROOT)}: "
            f"{', '.join(missing)}. Run `uv run python -m invoke admin-screenshots` "
            f"from the repo root."
        )
    for name in SITE_SCREENSHOTS:
        shutil.copy2(SCREENSHOT_SRC / name, dest / name)
    print(f"  copied {len(SITE_SCREENSHOTS)} screenshot(s) → {dest.relative_to(REPO_ROOT)}")


def _load_state() -> dict[str, str]:
    if not STATE_FILE.exists():
        raise SystemExit(f"AWS state file not found at {STATE_FILE}. Run `invoke infra-up` first.")
    return json.loads(STATE_FILE.read_text())


@task
def assets(c: Context) -> None:
    """Copy brandkit SVGs + admin screenshots into the theme's static/img/."""
    _ensure_venv()
...
def deploy(c: Context) -> None:
    ...
    c.run(
        f"cd {shlex.quote(str(main_repo))} && uv run --no-sync python -m invoke docs docs-rust",
        pty=True,
    )
...
def publish(c: Context, message: str = "") -> None:
    ...
    if not message:
        raise SystemExit('publish needs a commit message: invoke publish --message "what changed"')
    ...
    add_cmd = (
        ...
        + " add -- "
        + " ".join(shlex.quote(p) for p in paths_to_add)
    )
