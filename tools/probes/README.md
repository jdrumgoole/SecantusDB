...
| `agg_expressions.py` | every aggregation EXPRESSION operator (143), 1- and 2-argument forms | 3968 cases. **python: 0 crashes, 5 wrong values, 50 code / 212 message**; **rust: 0 wrong values, 902 code / 6 message** (2026-09-02). First run 2026-08-31 had 274 crashes, 57 wrong values and 2288 code diffs. The five remaining wrong values are a last-digit Decimal128 rounding on the transcendentals. Of the Rust 902, ~900 are the engine deferring on a bad OPERAND across ~120 operators — a per-operator campaign, see `tasks/backlog.md` |
