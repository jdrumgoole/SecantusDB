...
| `index_result_sets.py` | does an INDEX change which documents come back | **0 of 1707 on BOTH servers** (2026-09-01). Compares the server against ITSELF, with and without the index, plus the `_id` SET against mongod — the Rust server was **12 of 15 curated / 53 of 1692** before the same four fixes landed there. The only probe here that is not primarily a mongod comparison, because 'the index dropped rows' is invisible to one: the query succeeds and the shape is right |
...
| `update_operators.py` | update operator semantics and errors | 36 shapes: **python 0, rust 2** (2026-09-02). The two are the missing `Plan executor error during update :: caused by ::` wrapper — same code and message body; the Rust update path has no parse-time/execution-time distinction. Filed in `tasks/backlog.md` |
...
| `agg_expressions.py` | every aggregation EXPRESSION operator (143), 1- and 2-argument forms | 3968 cases. **python: 0 crashes, 5 wrong values, 28 code / 80 message**; **rust: 0 wrong values, 117 code / 0 message** (2026-09-02). First run 2026-08-31 had 274 crashes, 57 wrong values and 2288 code diffs; the Rust column was 902 + 6 until the per-operator campaign that day took it to 117. The five remaining Python wrong values are a last-digit Decimal128 rounding on the transcendentals. What is left is characterised operator-by-operator in `tasks/backlog.md` §7 rather than counted — mostly Decimal128 arithmetic and two families that need mongod's `%g` value rendering |
