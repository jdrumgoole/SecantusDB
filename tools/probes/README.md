...
| `agg_expressions.py` | every aggregation EXPRESSION operator (143), 1- and 2-argument forms | **6628 cases** (was 3968 until the corpus was widened on 2026-09-03). **python: 0 crashes, 19 wrong values, 28 code / 14 message**; **rust: 0 wrong values, 91 code / 17 message**. The widening — infinities, NaN, signed zero, the int32/int64 boundaries, MinKey/MaxKey/Binary/Timestamp/Regex/Code, empty and nested containers — immediately found **13 crash-class bugs** and 6 wrong values on surfaces this corpus had swept tens of thousands of times. A value CLASS that is absent is invisible exactly the way a passing test is. A second pass on 2026-09-03 closed 29 more: `$type` of a `bson.Code` answered `string` (the type vocabulary had a FOURTH partial copy), the shared helper did not know `javascriptWithScope` at all (25 messages in one change), and signed zero was dropped by `$ceil` / `$floor` / `$trunc` / `$add` / `$avg`. All 20 remaining wrong values are the `Decimal128`-computed-in-float limitation.  A third pass (2026-09-05) split the message differences by WRAPPER vs BODY and found **83 whose body was byte-identical to mongod's** -- only the `Invalid $addFields :: caused by ::` prefix was missing, which no code comparison can see. 76 of those were `$ifNull` and `$setEquals` arity errors reaching the evaluator instead of the parser, and a fourth pass closed 45 more that were all `$rand` -- every wrong argument to it already produced the right code and sentence under the wrong wrapper. **What is left is a FLAT TAIL, not a cluster**: the 28 codes are 28 different operators with one shape each, and the 14 messages likewise, so the next session should expect one-at-a-time work rather than another rule. See `tasks/backlog.md` §7 |
