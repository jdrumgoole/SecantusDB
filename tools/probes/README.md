...
| `agg_expressions.py` | every aggregation EXPRESSION operator (143), 1- and 2-argument forms | 3968 cases. **python: 0 crashes, 5 wrong values, 0 code / 71 message**; **rust: 0 wrong values, 22 code / 0 message** (2026-09-03). First run 2026-08-31 had 274 crashes, 57 wrong values and 2288 code diffs; the Rust column was 902 + 6 until the per-operator campaign took it to 117 (2026-09-02) and two further passes to 22 (09-03). The five remaining Python wrong values are a last-digit Decimal128 rounding on the transcendentals. What is left is characterised in `tasks/backlog.md` §7 rather than counted: 19 Decimal128 transcendental/division shapes and 3 `$toDate` parse diagnostics, both deliberate |
