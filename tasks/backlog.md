...
- [ ] **`$sum`/`$avg`/`$max`/`$min` as expression operators — Rust-server port** — these now work as expression operators on the Python server (MongoDB 5.0+ feature, #537: array-or-scalar reduction over `$project`/`$addFields`/etc.), but the Rust core defers them to Python, so the embedded Rust server errors (defer → BadValue) on e.g. `{$project: {s: {$sum: "$arr"}}}` rather than computing. Port `_op_expr_sum`/`_op_expr_avg`/`_op_expr_max`/`_op_expr_min` (`expressions.py`) to the Rust expression evaluator (`expressions.rs`), reusing the group-accumulator width logic (`Num::Int`/`Long`/`Double`) so the int32/int64/double result type matches Python for parity; add them to the recognized-op list + dispatch. Keep the parity `_rust_eval`-defers contract until then.
- [ ] **`$bucketAuto` `granularity` rounding — algorithm identified, 90% hex-exact, needs mongod's series constants** — `granularity` is *validated* (non-string → 40261, unknown → 40257; #530) and a valid series is **rejected as unsupported** (code 2) rather than silently producing count-chunked (unrounded) boundaries. The remaining work — the actual boundary rounding — is now **well-understood, not blocked**:
  - **Boundary rule** (probed vs mongod 7.0.12): first bucket min = `roundDown(dataMin)`; every other boundary = `roundUp(bucketMax)` (last boundary = `roundUp(dataMax)`).
  - **`roundUp(v)` / `roundDown(v)` algorithm** (verified hex-exact for R5 and 1-2-5): normalize the series to `[0.1, 1.0)`; scale `abs(v)` into `[0.1, 1.0)` by repeated `*10` / `/10`, tracking the `multiplier`; `roundUp` = smallest normalized `s` with `s > n`, `roundDown` = largest normalized `s` with `s < n` (both **strict**); result = `s * multiplier`; if none, wrap (`roundUp` → `series[0]*multiplier*10`, `roundDown` → `series[-1]*multiplier/10`). This deterministically reproduces mongod's non-standard ULPs (e.g. R5 decade-0 `6.3` → `6.300000000000001`).
  - **Remaining blocker (1-ULP):** the exact f64 of each series constant is mongod-specific and can't be pinned by black-box probing. The Renard (`R*`) and E-series (`E*`) preferred numbers derive from `10^(k/n)` rounded to decimal, and mongod's stored constants + scale-back order carry per-series ULPs: R5's decade-0 `6.3` comes out `6.300000000000001` (one ULP above *both* literal `6.3` and `0.63*10`), while E6's `6.8` is exactly `0.68*10` — so neither `iso/10` nor decimal-literal normalization matches every series. A prototype (decade-normalized literals) hits **158/176 (90%)** hex-exact across R5/R10/1-2-5/E6 over multiple decades; the ~10% misses are all 1 ULP, from (a) the series-constant f64 and (b) `multiplier` accumulation across decades. `POWERSOF2` is a separate, exact case (largest/smallest power of 2 strictly below/above `v`).
  - **To finish:** lift the exact series f64 constants (or their construction — likely `pow(10.0, k/n)` for Renard/E-series) **and** the exact scale-back operation order from mongod's `granularity_rounder_preferred_numbers.cpp`, port to both engines, verify hex-exact against the `pymongo → SecantusDB` vs `→ mongod` oracle over a broad corpus, then drop the unsupported-rejection. Shipping <100% hex-exact is **not acceptable** — a 1-ULP boundary divergence fails the oracle, which is worse than the current honest reject for a database.
