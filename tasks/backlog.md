...
- [ ] **Three-droplet DigitalOcean benchmark: teardown modes still unexercised**: `crates/secantus-bench` is live-verified end to end (2026-08-21) — provisioning, VPC + firewall, SSH, cloud-init, release-binary deploy, the on-droplet source build (`--server-build source`, vendored WiredTiger included), the on-droplet agent build, both engines, result collection, and `destroy` teardown. **Still unexercised**: the `snapshot` and `power-off` teardown modes (`destroy` is the default and is verified), and there is no repeat-and-median mode — one pass per engine per run, though two passes reproduced the headline ratio to within 1%. See `bench/DO_CLUSTER.md`.
