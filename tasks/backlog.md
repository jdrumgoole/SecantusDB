...
- [ ] **Sort / compound / descending / `hint`** — single-field equality (`$eq`, `$in`) and range (`$gt`/`$gte`/`$lt`/`$lte`) now accelerate through the WT B-tree using the typed sort-key encoder in `secantus.sortkey`. Unique enforcement is O(log n + k) instead of O(n). What's still missing: `sort: {indexed_field: 1}` walking the index in order, compound-index leading-prefix lookup, descending-direction indexes flipping the sort bytes, and `hint` actually picking an index. None of these need new encoding work — the encoder already produces byte-sortable bytes.
