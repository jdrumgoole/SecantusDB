...
- [ ] **Compound / native-descending / `hint`** — single-field equality (`$eq`, `$in`), range (`$gt`/`$gte`/`$lt`/`$lte`), and sort-by-indexed-field (both `+1` and `-1`, the latter via list reversal) now go through the WT B-tree using the typed sort-key encoder in `secantus.sortkey`. Unique enforcement is O(log n + k). What's still missing: compound-index leading-prefix lookup, native descending-direction indexes (today only `{field: 1}` indexes are supported), and `hint` actually picking an index. None need new encoding work.
