...
- [ ] **`$bucketAuto` `granularity` unimplemented** — the `buckets` argument is now fully validated (bool/non-numeric → 40241, fractional → 40242, non-positive → 40243, missing → 40246; whole-double accepted — #526), but the optional `granularity` field (`R5`/`R10`/…/`POWERSOF2`/`1-2-5`/`E6`…) is silently ignored: SecantusDB does the pure count-chunking without rounding bucket boundaries to the preferred-number series. A valid `granularity` is therefore accepted but produces mongod-divergent boundaries, and an *invalid* one (`granularity: 'BOGUS'` → mongod 40257; non-string → 40261) is not rejected. A dedicated slice should implement the rounding (both engines) and add the 40257/40261 validation; until then leave `granularity` out of any conformance claim.
