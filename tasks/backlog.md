...
- [ ] **Three-droplet DigitalOcean benchmark: no repeat-and-median mode**: `crates/secantus-bench` is fully live-verified (2026-08-21) — provisioning, VPC + firewall, SSH, cloud-init, release-binary deploy, the on-droplet source build (`--server-build source`), the on-droplet agent build, both engines, result collection, and all three teardown modes (`destroy`, `power-off` resume in ~1 min with IPs and software intact, `snapshot` image+destroy in ~1m44s then restore in ~2m09s with binaries still executable, plus `--purge-snapshots`). **Remaining gap**: one pass per engine per run — two passes reproduced the headline ratio to within 1%, but there is no built-in repeat-and-median mode, so a marginal result has to be re-run by hand. See `bench/DO_CLUSTER.md`.
- [ ] **SecantusDB is 0.46x mongod on a networked mixed workload (2026-08-21)**: the first three-droplet comparison on identical `c-4` hardware measured secantusd-rs 0.5.3-beta.160 at **8,244 ops/s vs mongod 8.0.29's 17,984** (8 KiB docs, 70/20/10 insert/find/update, 16 workers x 2 client droplets, 4G WT cache both). Latency is where it hurts: 1.57x at p50, **3.61x at p99, 9.84x at p99.9** — the tail, not the median, is the problem. Both engines were server-CPU-bound at ~84% with clients idle at 12-15%, so this is a real engine gap, not a harness artifact, and it reproduced within 1% on a second pass. Reproduce with `invoke do-bench`. Relates to the write-path gap in §7.
