...
- [ ] **Three-droplet DigitalOcean benchmark: never run against a live account**: `crates/secantus-bench` (the `do-cluster` orchestrator + `do-client` load agent) provisions a server + two client droplets, deploys `secantusd-rs`, runs a coordinated load test and tears the cluster down. The load agent, its hand-rolled `secantus-wire` client, the cross-machine histogram merge and the report were verified end-to-end against a local `secantusd-rs` (pymongo confirmed every reported insert reached storage); the DigitalOcean provisioning / deploy / teardown paths are written against the documented v2 API but have **not** been exercised for real — no token was available. First live run should expect to fix small API-shape details (droplet-create response fields, snapshot action timing, firewall PUT semantics). Also unexercised: `--server-build source` (the on-droplet WiredTiger + cargo build) and the on-droplet agent build. See `bench/DO_CLUSTER.md`.
