...
- [ ] **Compound-prefix range / native-descending / `hint`** — single-field equality (`$eq`, `$in`) and range (`$gt`/`$gte`/`$lt`/`$lte`), compound-index bare-equality leading-prefix lookup, and sort-by-indexed-field (both `+1` and `-1`) now go through the WT B-tree using the typed sort-key encoder in `secantus.sortkey`. Unique enforcement is O(log n + k). Still missing: range/`$in` operator forms against a **compound** prefix (single-field with operator forms still works), native descending-direction indexes (today only `{field: 1}` indexes are supported; `-1` sort reverses in Python), and `hint` actually picking an index. None need new encoding work.
