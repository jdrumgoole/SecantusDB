...
- [ ] **`$lookup` index acceleration: multikey foreign indexes still hash-join.** The simple-form / pipeline-with-prefilter paths drive the per-outer-doc lookup through `Storage.find_matching` whenever the foreign collection has a non-multikey index whose leading field is `foreignField` — single-field indexes and compound indexes (leading-field equality, prefix-scanned) both light up. When the foreign field is array-valued the index is flagged multikey and disqualified: each per-outer-doc lookup would force a full scan and the join becomes O(M*N) — strictly worse than the existing in-memory hash-join. A real fix needs per-element index entries on multikey indexes; deferred until the storage layer grows that.
