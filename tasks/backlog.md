...
- [ ] **`arrayFilters` nested-identifier extraction** — `arrayFilters` are now validated (non-object → 14, empty → 9, bad identifier → 2, duplicate → 9, unused → 9; #528), but the identifier is only extracted from a filter's *top-level* non-`$` field name. A filter whose only top-level keys are `$`-operators (e.g. `{$and: [{x: {...}}]}`, `{$or: […]}`) carries the identifier `x` *nested* inside the expression; SecantusDB doesn't walk into `$and`/`$or`/`$nor` to find it. Such a filter is left unvalidated (not wrongly rejected), but the apply path then can't resolve the `$[x]` path and errors ("arrayFilters has no entry for identifier x") where mongod succeeds. A dedicated slice should extract identifiers recursively through the logical operators (both `_validate_array_filters`/`_index_array_filters` in `update.py` and `array_filters_valid`/`index_array_filters` in `update.rs`).
- [ ] **`$bucketAuto` `granularity` rounding unimplemented** — `granularity` is now *validated* (non-string → 40261, unknown → 40257; #530) and a valid series (`R5`/…/`POWERSOF2`/`1-2-5`/`E6`…) is **rejected as unsupported** (code 2) rather than silently producing count-chunked (unrounded) boundaries. The remaining work is the actual boundary rounding. The rounding *rule* is known (probed vs mongod 7.0.12): the first bucket's min = the largest series value **strictly less** than the data min; every other boundary = the smallest series value **strictly greater** than the enclosing bucket's max. The blocker is **byte-exact float parity**: mongod's series constants land on non-standard f64 ULPs (its `6.3` is `6.3000000000000007`, not `float("6.3")` = `6.2999999999999998`), computed by internal arithmetic not recoverable by black-box probing — so a naive ISO-series implementation would differ from mongod at the ~1e-15 level and fail the `pymongo → SecantusDB` vs `→ mongod` oracle on exact-boundary inputs. A real slice needs mongod's exact series float construction (from its source) ported to both engines, then can drop the unsupported-rejection.
