...
- [ ] **Native-descending indexes** — single-field equality (`$eq`, `$in`) and range (`$gt`/`$gte`/`$lt`/`$lte`), compound bare-equality leading-prefix lookup, compound prefix + trailing-field operator (range / `$in` / `$eq`), sort-by-indexed-field (`+1` and `-1`), and user-supplied `hint` (name, key spec, `$natural`, or `_id_`) all go through the WT B-tree using the typed sort-key encoder in `secantus.sortkey`. Unique enforcement is O(log n + k). Still missing: native descending-direction indexes (today only `{field: 1}` indexes are supported; `-1` sort reverses the result list in Python — fine for typical sizes, costly for large reversed result sets). The encoder already produces byte-sortable bytes; this is wiring work.
- [ ] **`hint` for `aggregate`** — `find` honors `hint` end-to-end. The aggregation pipeline initial fetch (`_aggregate` → `find_matching(coll, {})`) does not yet plumb a `hint` field through, so `coll.aggregate([...], hint=...)` is silently ignored. Add it.
