...
- [ ] **mongo-c-driver (`libmongoc`) gauge landed (2026-06-19) — conformance gaps surfaced, not yet fixed.** New `c_validation` gauge builds the vendored driver's `test-libmongoc` from source (CMake) and runs a curated set of wire-protocol suites against an embedded daemon over `MONGOC_TEST_URI`. Driver pinned to `1.30.8`. Current: **707 pass / 26 fail / 69 skip (96.5% raw)**; the failure set is byte-identical across repeated full runs (deterministic, not flaky). 8 of the 26 are documented in `validation_summary/expected_failures.py` (`C` list — the RS-primary `hello` advertisement makes libmongoc's standalone/secondary server-type and `lastWriteDate`-absent assertions fail; IPv6 needs a v6 listener). The remaining ~18 are real divergences to chase (none block the gauge):
  - **`maxMessageSizeBytes` split boundary** (`/BulkOperation/OP_MSG/max_msg_size`, "2 == 1") — a bulk write the C driver expects to split into 2 OP_MSGs goes out as 1; SecantusDB's advertised max-message-size / write-batch limits don't force the split.
  - **`bypassDocumentValidation` on aggregate `$out`** (`/Collection/aggregate/bypass_document_validation`) — the test expects a cursor error; SecantusDB doesn't surface one (no document-validation enforcement on the `$out` path).
  - **getMore error-document shape** (`/Cursor/error_document/getmore`) — driver expects `mongoc_cursor_next` to stop on the error doc; SecantusDB's getMore error reply diverges.
  - **Decimal128 `batchSize`** (`/Cursor/batchsize_override_decimal128`) — a `batchSize` given as Decimal128 isn't honoured.
  - **Capped-collection tailable timeout** (`/Collection/tailable/timeout/single`) — same capped-tailable gap as the php-ext `cursor-tailable_error` note above (§4: capped tailables deferred).
  - **Namespace-length enforcement** (`/long_namespace/unsupported_long_db`) — SecantusDB accepts an over-long database name that mongod rejects.
  - **`collMod` error response** (`/collection-management/modifyCollection-errorResponse`) — error-reply shape for an invalid `collMod` diverges.
  - **`writeConcernError` reporting** (`/Client/command_w_write_concern`, `/Collection/index_w_write_concern`, `/Database/create_with_write_concern`, `/command_monitoring/unified/writeConcernError`) — write-concern error surfacing / APM events differ (single-node `w` handling). Likely the same single-node-topology class as the Ruby `w:2` note; not yet confirmed per-test.
  - **State-ordering-dependent drop/rename/create** (`/Collection/drop`, `/Collection/rename`, `/Collection/index`, `/Database/drop`) — pass in isolation, fail deterministically in the full-run ordering. Cause not yet isolated (a preceding test's side effect on the shared daemon); deterministic, so the gauge isn't flaky, but worth root-causing.
...
- [ ] **mongo-cxx-driver (`mongocxx`) gauge landed (2026-06-19) — conformance gaps surfaced, not yet fixed.** New `cxx_validation` gauge builds the vendored libmongoc (installed to a prefix) + the mongocxx `test_driver` Catch2 binary from source and runs it against an embedded daemon. Drivers pinned: mongocxx `r3.11.0` on libmongoc `1.30.8`. mongocxx's core tests hard-wire `mongodb://localhost:27017` (no `MONGOC_TEST_URI`-style override), so the gauge binds the daemon on **27017** (refuses to run if occupied). Current: **880 pass / 9 fail / 9 skip (99.0%)** — deterministic across repeated runs (Catch2 expands SECTIONs, so the testcase total > TEST_CASE count). The 9 failures are real divergences to chase (none block the gauge):
  - **Duplicate-index conflict not rejected** (`create_index tests/fails`, `create_one/fails for same keys and options`, `create_one/fails for same name`) — creating an index whose key/name collides with an existing one but with different options should raise `operation_exception`; SecantusDB accepts it. The strongest signal of the batch (3 tests).
  - **`$out` not last in pipeline not rejected** (`CRUD functionality/out fails when not last/aggregation`) — an aggregate with `$out` as a non-terminal stage should error; SecantusDB runs it.
  - **Change-stream invalid-pipeline error timing** (`Give an invalid pipeline/Error on .begin() even if no events`, `.../After error, begin == end repeatedly`) — opening a change stream with an invalid pipeline should error on `.begin()`; SecantusDB's error surfaces differently.
  - **Change-stream resume-token continuity** (`Spec Prose Tests/1. ChangeStream must continuously track the last seen resumeToken`) — `postBatchResumeToken` should advance even with no events; SecantusDB's token doesn't track as the spec expects (`token1 != token2`). Related to the §3.2 change-stream resume-token notes.
  - **Client-metadata handshake monitoring** (`integration tests for client metadata handshake feature/with client`, `/with pool`) — the test asserts the handshake `hello` op is observed via APM (`found_op`); SecantusDB's handshake/monitoring surface diverges.
...
- [ ] **mongo-csharp-driver (C# / .NET) gauge landed (2026-06-19) — one conformance gap surfaced.** New `dotnet_validation` gauge runs the vendored driver's xUnit `MongoDB.Driver.Tests` via `dotnet test` against an embedded daemon over `MONGODB_URI`. Driver pinned `v3.9.0`; scoped to the **CRUD specification suite** (`MongoDB.Driver.Tests.Specifications.crud`) via `--filter` — `MongoDB.Driver.Tests` as a whole is enormous and dominated by non-server unit tests (LINQ/serialization) plus external-service suites (CSFLE/KMS, auth, Atlas Search, load balancing) and multi-node features (transactions, sessions, SDAM, retryable). Current: **201 pass / 1 fail / 26 skip (99.5%)** (the 26 are `[RequireServer]`/CSFLE-gated skips). The one failure is a genuine divergence:
  - **Document-validation error detail** (`CrudProseTests.WriteError_details_should_expose_writeErrors_errInfo`) — when a write fails a collection validator, mongod returns a rich `writeErrors[].errInfo.details` (`operatorName` / `specifiedAs` / `reason` / `consideredValue` / `consideredType`); SecantusDB returns a minimal `{ operatorName: "validator" }`. SecantusDB enforces validators but doesn't synthesise the per-operator failure detail. Worth filling in.
