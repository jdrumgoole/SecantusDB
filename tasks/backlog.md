...
- [ ] **PHP gauges landed (2026-06-15) — conformance gaps surfaced, not yet fixed.** Two new gauges: `php_ext_validation` (mongo-php-driver `.phpt`, the low-level C extension that wraps libmongoc — strictest wire-protocol gauge alongside Go) at **99.0%** (664/671 ran, 41 skipped) and `php_lib_validation` (mongo-php-library PHPUnit, the high-level `mongodb/mongodb` package) at **98.0%** (3029/3091 ran, 39 skipped). Submodules pinned to the installed extension version (driver `2.3.1`, library `2.3.0`); the ext gauge runs against the already-installed extension via `run-tests.php` (no rebuild). Real divergences to chase (none block the gauge feature):
  - **`explain` output shape** — php-lib `ExplainFunctionalTest` (find/count/delete/distinct/findAndModify/aggregate) asserts keys like `queryPlanner` / `executionStats` / `allPlansExecution` our `_explain` reply doesn't surface. Widening `explain` verbosity is the fix.
  - **`collMod` no-op return** — php-lib `ModifyCollectionFunctionalTest::testCollMod` + `DatabaseFunctionalTest::testModifyCollection` expect `collMod` to echo a numeric field (got `null`, want `3`).
  - **write-error response shape** — php-ext `writeError`/`writeResult` (getWriteErrors / getMessage / debug) and php-lib `BulkWriteFunctionalTest::testInserts`: the `writeErrors[].errmsg` / structure we return diverges from mongod's.
  - **cursor lifecycle** — php-ext `cursor-destruct-001` (killCursors on destruct) + `cursor-tailable_error-001` (collection dropped mid-tailable-iteration).
  - **`count` hint + `IndexInfo` 2dsphere** — php-lib `CountFunctionalTest::testHintOption`, `IndexInfoFunctionalTest::testIs2dSphere` (the `2dsphereIndexVersion` field).
