...
- [x] **RESOLVED (2026-09-06): a `MinKey` / `MaxKey` bound now reaches an absent
  field.** The range operators are TYPE-BRACKETED, and `MinKey` / `MaxKey` are
  the two bounds that escape the bracketing — so they must see a missing field,
  which mongod's query language treats as `null` (above MinKey, below MaxKey).
  The comparison skipped an absent field outright, so `{x: {$gt: MinKey()}}` and
  `{x: {$lt: MaxKey()}}` dropped every document lacking the field. Four shapes,
  both servers.

  The fix is one line per server — compare an absent field as null instead of
  skipping it — and it is safe for the ordinary bounds precisely BECAUSE they
  are bracketed: `{x: {$gt: 3}}` then compares null against a number, the
  brackets differ, and the document is dropped exactly as before.

  Verified with an INDEX as well as without, which mattered: the fix makes these
  bounds return documents with no such field, exactly the class an index might
  omit. Both servers are byte-identical to mongod across no index, a regular
  index and a sparse index — including the sparse column, where mongod
  legitimately DOES omit the missing-field document.

  Pinned by `tests/test_minkey_maxkey_bounds.py` (27 tests) and two
  `secantus-core` unit tests. The read-path sweep went 14 -> 8 divergences on
  the Python server and 15 -> 9 on the Rust one.
...
- [x] **RESOLVED (2026-09-06, #1336): a dotted sort key now walks one array
  level.** The sort resolved its path with `get_path`, which deliberately does
  not walk through an array — right for `$set` and projection, wrong here. The
  array-descending resolver already existed for INDEX key generation with
  mongod's semantics, one-level stop included, so the fix was to use it and pick
  the representative element the direction asks for. That also makes the
  in-memory sort agree with an index walk by construction; the test asserts an
  index does not change the order, in both directions. Pinned by
  `tests/test_sort_dotted_array_descent.py` and two `secantus-storage` tests.

  **This entry was RESTORED after being destroyed by the very PR that earned
  it.** Two backlog edits in one branch each replaced a slice delimited by
  `s.index(start) : s.index(some_later_marker)`, and the second slice swallowed
  the first's output along with the `sort x.0` entry below. Nothing failed — the
  edit reported success and the record simply vanished, so finished work read as
  unfinished. **Do not edit this file by slicing between two unanchored
  markers.** Replace one entry's exact span (headline to the next `- [ ]`), and
...
- [x] **RESOLVED 2026-08-31 (partly) — the FIRST operator-by-operator sweep of
  ...
  1- and 2-argument form, against mongod 8.2.11: **3884 cases, 57 wrong VALUES,
  2288 different codes, 689 message-only, and 274 CRASHES.** Command arguments,
  change streams, updates and findAndModify had all been swept; the largest
  operator family in the server never had.

  **Fixed in this slice — the wrong-answer half**, four defects, both servers:
  - **Every cross-type comparison answered false.** `$gt`/`$gte`/`$lt`/`$lte`
    used the host language's operators inside `try/except TypeError: False`, so
    `{$gt: ["abc", 1]}` was false where mongod says true. `$cmp` had the correct
    comparator all along and the four never used it.
  - **`$and`/`$or` iterated a non-array argument**, walking a STRING CHARACTER
    BY CHARACTER (`"$s"` -> `'$'`, `'s'`). Short-circuiting kept: mongod DOES
    short-circuit at runtime, though an all-constant version folds and raises,
    which is why probing this needs a field reference.
  - **Truthiness**: every string is true, the EMPTY one included; missing is
    false. Both were wrong.
  - **A bool is not a number.** `{$eq: [true, 1]}` is false on mongod. The same
    root cause collapsed `$addToSet`'s `0` and `false`, made `{$in: [false, [0]]}`
    true, and in the oplog UPDATE-DIFF reported `true` -> `1` as no change, so a
    change stream never saw it.

  Two adjacent bugs fell out: `decimal.Decimal` was not ranked as a number in
  the BSON order (the SQL layer's numerics are native Decimals, so `price <
  cost * 1.5` compared by TYPE once the relational operators started using that
  order — caught by the SQL suite, not by the sweep), and **booleans were
  excluded from the Rust engine's sortable set** so every comparison involving
  one deferred to a generic BadValue.

  Pinned by 30 differential cases. `ordering.bson_equal` is now the single
...
- [x] **`maxTimeMS` diverged from mongod 8.2.1 in all four of its behaviours,
  ...
  "probed 6.0.16" re-probe, fixed by sweeping every command that takes the slot.
  The filed entry understated it: `_require_max_time_ms` was called from `find`
  alone, so on the other 23 commands a wrong-typed value was **silently
  accepted** — `{aggregate, pipeline: [], cursor: {}, maxTimeMS: "x"}` ran and
  reported success. That is the silently-accepted class, not the wrong-code
  class it was filed as. It is a generic command field in mongod's IDL, so the
  check now runs once in `dispatch` beside readConcern / apiVersion.

  The three rules the entry warned would be awkward were settled by probing 24
  commands (436 cases, 0 divergences against 8.2.1), and each came out simpler
  or different than expected:

  * **The struct name IS the command name — for 23 of 24.** `find` is the lone
    exception (`FindCommandRequest`), so it is a one-entry lookup table. The
    entry's "do not derive the struct name from the command name" was right
    about `find` and wrong about everything else.
  * **The type LIST is the same set everywhere** (`decimal, double, int, long`),
    rendered in **12 different orders** across the 24 commands — arbitrary IDL
    hash order, and it already reorders between *patch* builds (CLAUDE.md). Only
    the set is meaningful; we emit one fixed order and the tests assert a set.
  * **Check order is load-bearing**: `-1.5` is both non-integral and negative and
    mongod answers 9, not 2. A fractional `Decimal128` also gets *different*
    wording from a fractional `double` ("Cannot represent as a 64-bit integer"
    vs "Expected an integer") for the same numeric value.

  Two more facts were measured rather than guessed, because both orderings were
  plausible and one of them would have been wrong: **CommandNotFound (59) beats
  the check** (an unknown command with a bad `maxTimeMS` answers 59), while **the
  check beats authorization (13)** — probed against an auth-enabled mongod. That
  fixes its position in `dispatch` exactly: below the handler lookup, above the
  auth checks. Also found: an upper bound of 2147483647 that was not enforced at
  all. Pinned by `tests/test_max_time_ms_fidelity.py` and 109 new cases in the
  differential gate; the four tests in `test_arg_types_accepted_slots.py` that
  asserted the 6.0 contract were rewritten (the "tests that pin a limitation"
  class again).

  **Fixed on BOTH servers.** The Rust server had ported the same 6.0 contract —
  including the doc comment asserting the false claim, which is the "comment
  justifying behaviour by something other than the oracle" shape again — and
  called it from `find` / `aggregate` / `findAndModify` only, so 21 commands
  were silent there too. No existing suite would have caught this drift: the
  engine-parity suites cover the operator engines, not the command layer.

- [x] **`createIndexes` with `indexes: null` answered the wrong code — FIXED
  2026-08-30.** Now `40414 IDLFailedToParse: BSON field 'createIndexes.indexes'
  is missing but a required field`, byte-identical to omitting the field, where
  we answered the 6.0 form `10065 invalid parameter: expected an object
  (indexes)`. Same null-means-absent family as `findAndModify.arrayFilters` and
  `killCursors.cursors`; missed because the crash here was fixed separately
...
- [x] **RESOLVED (2026-09-01): `maxTimeMS` is ENFORCED on the Python server.**
  ...
  **`maxTimeMS` is validated but still not ENFORCED (measured 2026-08-30).**
  ...
  failed: ... :: caused by :: operation exceeded time limit"), we run to
  completion and answer ok. Visible in a mongod diff only when the value is small
  enough for the operation to actually exceed it — a 436-case sweep showed 0
  divergences at `maxTimeMS: 1000` and 5–11 (varying by run, since it is a race
  against real elapsed time) at `maxTimeMS: 2`. Enforcing it means a deadline
  threaded through the command handlers, not a parse-time check. Pre-existing;
  the "accepted but not enforced" note at `commands.py` ~line 1990 is this.
...
- [x] **RESOLVED 2026-08-31 — six `_secantus_server`-gated test files ran
  nowhere; all six now run, and doing so found two real Windows defects.**
  They skip in every ordinary lane (no other job builds that extension), and
  the one job that does — `storage-engine` — runs a list of NAMED files. Six
  were not on it, including two data-integrity suites (crash recovery,
  cross-server PITR) and the file pinning the 76 argument slots fixed the same
  day. Nothing failed; they simply never ran.

  The list is named rather than a marker sweep for a real reason: that job
  installs a minimal environment (pymongo + pytest + the storage wheel) where
  collecting all of `tests/` would fail on imports. So the fix was not to
  replace it but to stop it drifting —
  `tests/test_ci_runs_rust_server_tests.py` now fails the suite when a
  Rust-server file is neither wired in nor given a written reason.

  **Two things measurement corrected, both of which reading would have got
  wrong:**

  * The concurrency step's comment says the storage-flavoured wheel does not
    ship the SWIG `wiredtiger` binding. **It does** — `CMakeLists.txt` builds
    it with `ENABLE_PYTHON=ON` and installs it into the wheel, and all six
    files import and run there. Believing the comment would have excluded four
    files for a reason that is not true. (That comment is still wrong and is
    worth correcting where it sits.)
  * `test_crash_recovery.py` had **never run on Windows**, and hid two defects:
    `signal.SIGKILL` does not exist there (now `Popen.kill()`, which is SIGKILL
    on POSIX and TerminateProcess on Windows — both uncatchable, no cleanup, so
    the simulated crash is the same), and reopening the store immediately after
    the kill answers `WtError code 16` (EBUSY) while the OS releases the dead
    process's handles.

  **The EBUSY is a wait, not a wall**: with a bounded 30s retry that tolerates
  only a busy-lock, Windows goes from `2 failed, 160 passed` to `162 passed`.
  So `secantusd-rs` does restart after a hard kill on Windows — it just cannot
  be reopened in the same instant. Worth knowing for anyone writing restart
  tooling; a real crash-restart faces the same wait.

  Left deliberately: nothing. `DELIBERATELY_NOT_IN_CI` is empty, and the
  stress suite that prompted the "CI budget" caveat runs in 2 minutes with the
  rest.
