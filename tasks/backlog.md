...
- [ ] **More aggregation expressions**: `$function` (JS — also out of scope). `$mergeObjects` is already implemented (see `expressions.py:1443`).
- [ ] **More aggregation stages**: `$merge` (writes pipeline output back into a collection — distinct from the `$mergeObjects` *expression* which is implemented), `$fill`.
