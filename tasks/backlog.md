...
- **Geo (operators + indexes)** — `$geoWithin` / `$geoIntersects` / `$near` / `$nearSphere` operators, `$geoNear` aggregation stage (with `key` auto-infer from the first geo index when omitted, and `includeLocs` echoing the raw doc geometry under a named field), plus `2dsphere` (S2 cell coverings + ancestors) and `2d` (bit-interleaved geohash buckets) index acceleration. See `src/secantus/geo.py` + `src/secantus/geo_index.py`. **Conformance gauge note**: pymongo's in-scope sync test set has dedicated geo coverage in exactly one place (`test_collection.py::test_index_geo2d` + `test_index_2dsphere`) which were already passing. There is no widening to do via `pymongo_validation/include_paths.py` — the gauge can't measure the operator surface gain; that's covered by `tests/test_geo_query.py` + `test_geo_index.py` in-tree (48 tests). **Cross-driver smoke**: `tests/test_geo_cross_driver.py` runs the canonical `$geoWithin` / `$geoNear` workload through mongosh, mongo-node-driver, and mongo-go-driver — all three pass, so the wire-protocol geo path is clean across drivers. Java is *not* covered cross-driver because a Java smoke needs Maven/Gradle scaffolding (no single-file equivalent like `node geo_smoke.js` or `go run .`); BSON-level GeoJSON serialization is exercised by the `:bson:test` Java gauge already. **Remaining (real, not nice-to-have)**: fine-grained 2d range covering for large query polygons. The current single-coarse-bbox range over-scans Z-order space because bit-interleaved geohashes don't preserve row contiguity — a proper fix needs Tropf-Herzog LITMAX/BIGMIN-style range decomposition (~100+ LOC), not the trivial enhancement the previous wording implied. The verifier filters all false positives, so this is a perf optimisation not a correctness gap; defer until a workload actually hits it.
