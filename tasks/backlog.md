...
- [ ] **Native-descending indexes / `hint`** — single-field equality (`$eq`, `$in`) and range (`$gt`/`$gte`/`$lt`/`$lte`), compound bare-equality leading-prefix lookup, compound prefix + trailing-field operator (range / `$in` / `$eq`), and sort-by-indexed-field (`+1` and `-1`) now all go through the WT B-tree using the typed sort-key encoder in `secantus.sortkey`. Unique enforcement is O(log n + k). Still missing: native descending-direction indexes (today only `{field: 1}` indexes are supported; `-1` sort reverses in Python), and `hint` actually picking an index. None need new encoding work.
